...
const AIStoryboardNodeInner: React.FC<AIStoryboardNodeProps> = ({
  ...
  const timelineEmptyStateText = imageCount > 0
    ? '当前还没有镜头。可以点击 AI 编排，或先手动添加一个空白镜头。'
    : '先接入上游图片，或直接添加空白镜头开始搭建分镜。';

  const storyboardViewProps = useMemo<StoryboardShotTimelineViewProps>(() => ({
    shots: storyboardState.shots,
    previews: shotPreviewMap,
    emptyStateText: timelineEmptyStateText,
  ...
  return (
    <div
      ...
      data-storyboard-legacy-empty-state={emptyStateText}
    >
      {!data.locked ? (
        <>
          <button
