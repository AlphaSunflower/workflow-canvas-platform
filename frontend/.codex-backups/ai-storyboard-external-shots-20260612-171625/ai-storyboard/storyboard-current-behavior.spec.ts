...
test('current execution payload keeps shot id as group id and routes output through node-level handle', () => {
  const node = createStoryboardNode();
  ...
  assert.equal(imagePayload.targets[0]?.outputHandle, getAIStoryboardOutputHandle('group-1'));
  assert.equal(videoPayload.targets[0]?.outputHandle, getAIStoryboardOutputHandle('group-1'));
});
...
test('current adapter extracts image and video outputs onto the storyboard node-level output handle', () => {
  ...
  assert.equal(imageOutputs[0]?.sourceHandle, getAIStoryboardOutputHandle('group-1'));
  assert.equal(videoOutputs[0]?.sourceHandle, getAIStoryboardOutputHandle('group-1'));
  assert.equal(imageOutputs[0]?.groupOrder, 0);
  assert.equal(videoOutputs[0]?.groupOrder, 0);
});
...
test('current adapter commits each storyboard output as a distinct right-side file node', async () => {
  ...
  const outputLinks = workflow.connections.filter((connection) => (
    ...
    && connection.sourceHandle === getAIStoryboardOutputHandle('group-1')
  ));
  ...
  assert.equal(
    orderedOutputNodes.every((outputNode) => outputNode.position.x === orderedOutputNodes[0]?.position.x),
    true,
  );
...
test('current committed-state check treats stale storyboard snapshots as already handled', async () => {
  ...
  node.tasks = [{
    ...
    scope: 'group',
    groupId: shot.id,
    outputHandle: getAIStoryboardOutputHandle('group-1'),
    status: 'completed',
    createdAt: 2,
  }];
