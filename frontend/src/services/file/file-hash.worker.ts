...
self.addEventListener('message', (event: MessageEvent<WorkerHashRequest>) => {
  const payload = event.data;

  void hashFile(payload.file)
    .then((sha256) => {
      const message: WorkerHashResponse = {
        id: payload.id,
        success: true,
        sha256,
      };
    ...
    .catch((error) => {
      const message: WorkerHashResponse = {
        id: payload.id,
        success: false,
        error: error instanceof Error ? error.message : 'Failed to hash workflow file.',
      };
      self.postMessage(message);
    });
});
