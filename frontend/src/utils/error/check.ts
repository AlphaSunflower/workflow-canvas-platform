...
export function isRecoverable(error: AppError): boolean {
  const info = getErrorInfo(error.code as ErrorCode);
...
export function isFileError(error: AppError): boolean {
  ...
  return fileCodes.includes(error.code as ErrorCode);
}
 ...
 * @returns 是否为AI任务错误
 */
export function isAITaskError(error: AppError): boolean {
  const aiCodes: ErrorCode[] = [
    'AI_TASK_ERROR',
    'AI_TASK_TIMEOUT',
    'AI_TASK_CANCELLED',
    'AI_PROVIDER_ERROR',
  ...
  return aiCodes.includes(error.code as ErrorCode);
}
