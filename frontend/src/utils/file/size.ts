...
export function formatFileSize(bytes: number, decimals: number = 2): string {
  ...
  const size = bytes / Math.pow(k, clampedI);
  
  return `${size.toFixed(clampedI > 0 ? decimals : 0)} ${units[clampedI]}`;
}
 ...
 * // 1572864
 */
export function parseFileSize(formatted: string): number {
 ...
 * @returns 平均字节数
 */
export function calculateAverageSize(files: File[]): number {
 ...
 * @returns 分类标签
 */
export function getFileSizeCategory(bytes: number): 'tiny' | 'small' | 'medium' | 'large' | 'huge' {
  const mb = bytes / (1024 * 1024);
  
  if (mb < 0.1) return 'tiny';    // < 100KB
  if (mb < 1) return 'small';     // < 1MB
  if (mb < 10) return 'medium';   // < 10MB
  if (mb < 100) return 'large';   // < 100MB
  return 'huge';                   // >= 100MB
}
 ...
 * @returns 比较结果
 */
export function compareFileSize(fileA: File, fileB: File): number {
