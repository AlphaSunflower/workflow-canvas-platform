...
test('storyboard execution payload keeps shot group id and per-shot output handle', () => {
  const { workflow, nodeId } = createWorkflowWithStoryboardNode();
  ...
  assert.equal(payload.targets[0]?.outputHandle, getAIStoryboardShotOutputHandle('shot-1'));
...
test('storyboard task ref merge keeps image and video refs for the same shot output handle separated by task type', () => {
  const outputHandle = getAIStoryboardShotOutputHandle('shot-1');
  const merged = mergeStoryboardNodeTaskRefs([
    {
      taskId: 'task-image-1',
