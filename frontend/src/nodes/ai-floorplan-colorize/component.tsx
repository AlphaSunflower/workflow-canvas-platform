...
import {
  AI_FLOORPLAN_COLORIZE_DEFAULT_GROUP_HEIGHT,
  AI_FLOORPLAN_COLORIZE_DESCRIPTION,
  AI_FLOORPLAN_COLORIZE_DISPLAY_NAME,
  AI_FLOORPLAN_COLORIZE_GROUP_STACK_GAP,
  ...
  AI_FLOORPLAN_COLORIZE_MIN_HEIGHT,
  AI_FLOORPLAN_COLORIZE_MIN_WIDTH,
  AI_FLOORPLAN_COLORIZE_MODEL_OPTIONS,
  ...
  normalizeAIFloorplanColorizeModel,
  normalizeAIFloorplanColorizeStylePreset,
} from './constants';
import { NodeInputImagePreview } from '../shared/NodeInputImagePreview';
import {
  getAIFloorplanColorizeInputHandle,
  getAIFloorplanColorizeOutputHandle,
} from './groups';
import type { AIFloorplanColorizeDropSide } from './drop-config';

interface AIFloorplanColorizeNodeProps extends NodeProps<AINodeData> {}
...
function getEdgeConnectionType(edge: ReactFlowEdge): 'file-reference' | 'output-link' {
  const connectionType = (
    edge.data as { connectionType?: 'file-reference' | 'output-link' } | undefined
  )?.connectionType;
...
function useNodeStructuralEditor(nodeId: string): (updater: (node: AINodeData) => AINodeData) => void {
  ...
  return useCallback((updater: (node: AINodeData) => AINodeData): void => {
    ...
    setNodes(nextNodes);
    actions.syncRuntimeSnapshot(
      createWorkflowRuntimeSnapshot(nextNodes, getEdges(), getViewport())
    );
  }, [actions, getEdges, getNodes, getViewport, nodeId, setNodes]);
}

const AIFloorplanColorizeNodeInner: React.FC<AIFloorplanColorizeNodeProps> = ({
  data,
  selected,
}) => {
  const {
  ...
  const { inputRegionProps, getPanelTargetProps, getSlotTargetProps } =
    useGroupedDropInteraction<AIFloorplanColorizeDropSide>();

  const nodeInfo = NODE_TYPE_INFO[data.type];
  const nodeColor = nodeInfo?.color || '#f97316';
  const nodeIcon = nodeInfo?.icon || 'CLR';
  ...
  const normalizedImageConfig = useMemo(() => normalizeAIFloorplanColorizeConfig({
  ...
  const aspectRatioOptions = useMemo(
    () => getAIFloorplanColorizeAspectRatioOptions(normalizedImageConfig.model),
    [normalizedImageConfig.model],
  );
  const usesParameterControls = !isAIFloorplanColorizeParameterlessModel(normalizedImageConfig.model);
  ...
  const groupsWithExecution = useMemo(() => resolvedGroupStates.map((groupState) => ({
    ...
    inputs:
      groupState.ports.find((port) => port.portId === AI_FLOORPLAN_COLORIZE_INPUT_PORT_ID)
        ?.inputs ?? [],
  ...
  const inputLayoutVersion = useMemo(
    () => groupsWithExecution
      .map(({ group, inputs, execution }) => [
        group.id,
        inputs.map(({ sourceNode }) => sourceNode.id.value).join(','),
        execution?.status ?? '',
        execution?.progress ?? '',
        execution?.message ?? '',
        execution?.error ?? '',
  ...
  const getGroupStackOffset = useCallback((groupElement?: Element | null): number => {
    ...
    return Math.round(measuredHeight + AI_FLOORPLAN_COLORIZE_GROUP_STACK_GAP);
  }, [getViewport]);
  ...
  const handleDelete = useCallback((event: React.MouseEvent<HTMLButtonElement>): void => {
    ...
    const relatedConnections = currentEdges.filter(
      (edge) => edge.source === data.id.value || edge.target === data.id.value
    );

    if (relatedConnections.length > 0) {
      const confirmed = window.confirm(
        `该节点存在 ${relatedConnections.length} 条连接，确认删除吗？`
      );
      if (!confirmed) {
        return;
      }
  ...
  const handleResizeStart = useNodeResizeInteraction<Partial<AINodeData>>({
    ...
    resolveUpdate: ({ screenDelta, zoom }) => ({
      dimensions: resolveNodeResizeDimensions({
        ...
        minDimensions: {
          width: AI_FLOORPLAN_COLORIZE_MIN_WIDTH,
          height: AI_FLOORPLAN_COLORIZE_MIN_HEIGHT,
        },
        screenDelta,
        zoom,
      }),
  ...
  const handleModelChange = useCallback((event: React.ChangeEvent<HTMLSelectElement>) => {
    const nextModel = normalizeAIFloorplanColorizeModel(event.target.value);
    const nextConfig = normalizeAIFloorplanColorizeConfig({
      model: nextModel,
      imageSize: normalizedImageConfig.imageSize,
      aspectRatio: normalizedImageConfig.aspectRatio,
    });
    ...
    updateImageConfig({
      ...
      ...(!isAIFloorplanColorizeParameterlessModel(nextConfig.model)
        ? {
  ...
  const handleStylePresetChange = useCallback((event: React.ChangeEvent<HTMLSelectElement>) => {
    updateImageConfig({
      stylePreset: normalizeAIFloorplanColorizeStylePreset(event.target.value),
    });
  }, [updateImageConfig]);

  const handleAddGroup = useCallback(() => {
    if (groups.length >= AI_FLOORPLAN_COLORIZE_MAX_GROUPS) {
      return;
    }
  ...
  const handleRemoveGroup = useCallback((
    ...
    if (groups.length <= AI_FLOORPLAN_COLORIZE_MIN_GROUPS) {
      return;
    }

    const stackOffset = getGroupStackOffset(
      event.currentTarget.closest('.ai-image-gen-node__group')
    );
    ...
    const nextEdges = currentEdges.filter((edge) => !(
      (edge.target === data.id.value &&
        edge.targetHandle === getAIFloorplanColorizeInputHandle(groupId)) ||
      (edge.source === data.id.value &&
        edge.sourceHandle === getAIFloorplanColorizeOutputHandle(groupId))
    ));

    const remainingOutputFileIds = new Set(
      nextEdges
        .filter((edge) =>
          edge.source === data.id.value && getEdgeConnectionType(edge) === 'output-link'
        )
        .flatMap((edge) => {
  ...
  const handleRemoveGroupInput = useCallback((
    ...
    const { nextNodes, nextEdges } = removeGroupedPortInput(getNodes(), getEdges(), {
      ...
      inputHandle: getAIFloorplanColorizeInputHandle(groupId),
      outputHandle: getAIFloorplanColorizeOutputHandle(groupId),
      sourceId,
    });
  ...
  const compositeWidth = Math.max(AI_FLOORPLAN_COLORIZE_MIN_WIDTH, data.dimensions.width);
  const compositeHeight = Math.max(AI_FLOORPLAN_COLORIZE_MIN_HEIGHT, data.dimensions.height);
          ...
          groups={(
            <div ref={groupLayerRef} className="ai-image-gen-node__groups">
              {groupsWithExecution.map(({ group, inputs, execution }) => {
                ...
                const canRemoveGroup = groups.length > AI_FLOORPLAN_COLORIZE_MIN_GROUPS;
                ...
                const dropzoneClasses = [
                  ...
                  hasInput
                    ? 'ai-image-gen-node__group-dropzone--filled'
                    : 'ai-image-gen-node__group-dropzone--empty',
                ].join(' ');
                ...
                return (
                  ...
                  >
                    <Handle
                      id={getAIFloorplanColorizeInputHandle(group.id)}
                      type="target"
                    ...
                    <Handle
                      id={getAIFloorplanColorizeOutputHandle(group.id)}
                      type="source"
                    ...
                    <div className="ai-image-gen-node__group-header">
                      <div className="ai-image-gen-node__group-header-main">
                        ...
                        {execution?.status && (
                          <span
                            className={`ai-image-gen-node__group-status ai-image-gen-node__group-status--${execution.status}`}
                          >
                    ...
                    <div
                      ...
                      {...getSlotTargetProps({
                        nodeId: data.id.value,
                        groupId: group.id,
                        portId: AI_FLOORPLAN_COLORIZE_INPUT_PORT_ID,
                        side: 'input',
                      })}
                    >
                      {sourceNode ? (
          ...
          toolbar={(
            <>
              <button
                ...
                disabled={groups.length >= AI_FLOORPLAN_COLORIZE_MAX_GROUPS}
                title={
                  groups.length >= AI_FLOORPLAN_COLORIZE_MAX_GROUPS
                    ? '最多 10 个输入组'
                    : '新增输入组'
                }
              >
      ...
      >
        {isProcessing && (
          <div className="ai-node__processing-overlay">
            <div
              className="ai-node__processing-spinner"
        ...
        {!data.locked && (
          <>
            ...
            <button
              ...
              title={isProcessing ? '取消运行' : canRun ? '运行节点' : '当前节点缺少有效输入'}
              onMouseDown={stopPointerEvent}
        ...
        >
          ...
          <span className="node-type-name">
            {nodeInfo?.displayName || AI_FLOORPLAN_COLORIZE_DISPLAY_NAME}
          </span>
          ...
          <div className="ai-image-gen-node__parameter-panel">
            <div className="ai-image-gen-node__parameter-grid">
              <label className="ai-image-gen-node__field ai-image-gen-node__field--compact">
                ...
                >
                  {AI_FLOORPLAN_COLORIZE_MODEL_OPTIONS.map((option) => (
              ...
              <label className="ai-image-gen-node__field ai-image-gen-node__field--compact">
                ...
                <select
                  className="select ai-image-gen-node__select nodrag nopan"
                  value={normalizeAIFloorplanColorizeStylePreset(data.config.stylePreset)}
                  onChange={handleStylePresetChange}
                >
                  {AI_FLOORPLAN_COLORIZE_STYLE_OPTIONS.map((option) => (
                    <option key={option.value} value={option.value}>
                      {option.label}
                    </option>
                  ))}
              ...
              <label className="ai-image-gen-node__field ai-image-gen-node__field--compact">
                ...
                >
                  {AI_FLOORPLAN_COLORIZE_IMAGE_SIZE_OPTIONS.map((option) => (
                    <option key={option.value} value={option.value}>
                      {option.label}
                    </option>
                  ))}
          ...
          {!data.locked && (
            <div className="ai-image-gen-node__footer-actions">
              <button
                ...
                title={
                  isProcessing ? '取消运行' : canRun ? '运行节点' : '当前节点暂不可运行'
                }
                onMouseDown={stopPointerEvent}
                onClick={isProcessing ? handleCancel : handleRun}
                disabled={!isProcessing && !canRun}
...
AIFloorplanColorizeNodeInner.displayName = 'AIFloorplanColorizeNodeInner';

export const AIFloorplanColorizeNode = memo((props: AIFloorplanColorizeNodeProps) => (
  <NodeErrorBoundary nodeId={props.data.id.value} nodeType={props.data.type}>
    <AIFloorplanColorizeNodeInner {...props} />
  </NodeErrorBoundary>
));

AIFloorplanColorizeNode.displayName = 'AIFloorplanColorizeNode';
