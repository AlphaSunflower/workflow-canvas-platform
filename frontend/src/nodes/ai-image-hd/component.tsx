...
const AIImageHdNodeInner: React.FC<AIImageHdNodeProps> = ({ data, selected }) => {
  const {
    actions,
    selectors,
  ...
  const { inputRegionProps, getPanelTargetProps, getSlotTargetProps } =
    useGroupedDropInteraction<AIImageHdDropSide>();

  const nodeInfo = NODE_TYPE_INFO[data.type];
  const nodeColor = nodeInfo?.color || '#0ea5e9';
  const nodeIcon = nodeInfo?.icon || 'HD';
  ...
  const normalizedImageConfig = useMemo(() => normalizeAIImageHdNodeConfig({
  ...
  const aspectRatioOptions = useMemo(
    () => getAIImageHdNodeAspectRatioOptions(normalizedImageConfig.model),
    [normalizedImageConfig.model],
  );
  const usesParameterControls = !isAIImageHdNodeParameterlessModel(normalizedImageConfig.model);
  ...
  const groupsWithExecution = useMemo(() => resolvedGroupStates.map((groupState) => ({
    ...
    inputs: groupState.ports
      .find((port) => port.portId === AI_IMAGE_HD_INPUT_PORT_ID)
      ?.inputs ?? [],
  ...
  const getGroupStackOffset = useCallback((groupElement?: Element | null): number => {
    ...
    return Math.round(measuredHeight + AI_IMAGE_HD_GROUP_STACK_GAP);
  }, [getViewport]);
  ...
  const handleResizeStart = useNodeResizeInteraction<Partial<AINodeData>>({
    ...
    resolveUpdate: ({ screenDelta, zoom }) => ({
      dimensions: resolveNodeResizeDimensions({
        ...
        minDimensions: {
          width: AI_IMAGE_HD_MIN_WIDTH,
          height: AI_IMAGE_HD_MIN_HEIGHT,
        },
        screenDelta,
        zoom,
      }),
  ...
  const handleModelChange = useCallback((event: React.ChangeEvent<HTMLSelectElement>) => {
    const nextModel = normalizeAIImageHdNodeModel(event.target.value);
    const nextConfig = normalizeAIImageHdNodeConfig({
      model: nextModel,
      imageSize: normalizedImageConfig.imageSize,
      aspectRatio: normalizedImageConfig.aspectRatio,
    });
    ...
    updateImageConfig({
      ...
      ...(!isAIImageHdNodeParameterlessModel(nextConfig.model)
        ? {
  ...
  const handleAddGroup = useCallback(() => {
    if (groups.length >= AI_IMAGE_HD_MAX_GROUPS) {
      return;
    }
  ...
  const handleRemoveGroup = useCallback((groupId: string, event: React.MouseEvent<HTMLButtonElement>): void => {
    ...
    if (groups.length <= AI_IMAGE_HD_MIN_GROUPS) {
      return;
    }
    ...
    const nextEdges = currentEdges.filter((edge) => !(
      (edge.target === data.id.value && edge.targetHandle === getAIImageHdGroupInputHandle(groupId)) ||
      (edge.source === data.id.value && edge.sourceHandle === getAIImageHdGroupOutputHandle(groupId))
    ));
  ...
  const handleRemoveGroupInput = useCallback((
    ...
    const { nextNodes, nextEdges } = removeGroupedPortInput(getNodes(), getEdges(), {
      ...
      inputHandle: getAIImageHdGroupInputHandle(groupId),
      outputHandle: getAIImageHdGroupOutputHandle(groupId),
      sourceId,
    });
  ...
  const compositeWidth = Math.max(AI_IMAGE_HD_MIN_WIDTH, data.dimensions.width);
  const compositeHeight = Math.max(AI_IMAGE_HD_MIN_HEIGHT, data.dimensions.height);
    ...
    >
          ...
          groups={(
            <div ref={groupLayerRef} className="ai-image-gen-node__groups">
              {groupsWithExecution.map(({ group, inputs, execution }) => {
                ...
                const canRemoveGroup = groups.length > AI_IMAGE_HD_MIN_GROUPS;
                ...
                return (
                  ...
                  >
                    <Handle
                      id={getAIImageHdGroupInputHandle(group.id)}
                      type="target"
                    ...
                    <Handle
                      id={getAIImageHdGroupOutputHandle(group.id)}
                      type="source"
                    ...
                    <div className="ai-image-gen-node__group-header">
                      ...
                      <div className="ai-image-gen-node__group-header-actions">
                        <span className="ai-image-gen-node__group-meta">{hasInput ? `${AI_IMAGE_HD_INPUT_LIMIT_PER_GROUP} / ${AI_IMAGE_HD_INPUT_LIMIT_PER_GROUP}` : `0 / ${AI_IMAGE_HD_INPUT_LIMIT_PER_GROUP}`}</span>
                        <button
                          type="button"
                          ...
                          title={canRemoveGroup ? 'Remove group' : 'At least one group is required'}
                        >
                          x
                        </button>
                      </div>
                    ...
                    <div
                      ...
                      {...getSlotTargetProps({
                        nodeId: data.id.value,
                        groupId: group.id,
                        portId: AI_IMAGE_HD_INPUT_PORT_ID,
                        side: 'input',
                      })}
                    >
                      {sourceNode ? (
          ...
          toolbar={(
            <>
              <button
                ...
                disabled={groups.length >= AI_IMAGE_HD_MAX_GROUPS}
                title={groups.length >= AI_IMAGE_HD_MAX_GROUPS ? '最多 10 个输入组' : '新增输入组'}
              >
                + 添加输入组
              </button>
      ...
      >
        ...
        <div className="node-header ai-image-gen-node__titlebar" style={{ backgroundColor: `${nodeColor}1f` }}>
          ...
          <span className="node-type-name">{nodeInfo?.displayName || AI_IMAGE_HD_DISPLAY_NAME}</span>
        ...
        >
          {/*
            ...
            <textarea
              ...
              placeholder="输入共享提示词，所有输入组共用同一段描述"
              rows={4}
            />
            </label>
          */}
          ...
          <div className="ai-image-gen-node__parameter-panel">
            <div className="ai-image-gen-node__parameter-grid">
              <label className="ai-image-gen-node__field ai-image-gen-node__field--compact">
                ...
                >
                  {AI_IMAGE_HD_NODE_MODEL_OPTIONS.map((option) => (
              ...
              <label className="ai-image-gen-node__field ai-image-gen-node__field--compact">
                ...
                >
                  {AI_IMAGE_HD_NODE_IMAGE_SIZE_OPTIONS.map((option) => (
                    <option key={option.value} value={option.value}>
                      {option.label}
                    </option>
                  ))}
...
AIImageHdNodeInner.displayName = 'AIImageHdNodeInner';

export const AIImageHdNode = memo((props: AIImageHdNodeProps) => (
  <NodeErrorBoundary nodeId={props.data.id.value} nodeType={props.data.type}>
    <AIImageHdNodeInner {...props} />
  </NodeErrorBoundary>
));

AIImageHdNode.displayName = 'AIImageHdNode';
