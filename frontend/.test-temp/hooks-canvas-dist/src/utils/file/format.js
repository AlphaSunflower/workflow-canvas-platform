...
export function getFileTypeFromExtension(extension) {
    var normalizedExt = extension.toLowerCase();
    var type = FILE_TYPE_MAP[normalizedExt];
    return type !== null && type !== void 0 ? type : null;
}
/**
 * 根据文件名获取文件类型
...
export function isModelFile(fileName) {
    var extension = getFileExtension(fileName);
    return SUPPORTED_FORMATS.model3d.includes(extension);
}
/**
 * 判断是否为支持的文件
 ...
 * @returns 是否需要转码
 */
export function needsTranscoding(mimeType) {
...
export function getFileIcon(fileType) {
    ...
    return (_a = icons[fileType]) !== null && _a !== void 0 ? _a : '📄';
}
/**
 * 根据文件名获取图标
 ...
 * @returns 图标emoji
 */
export function getFileIconFromName(fileName) {
 ...
 * @returns 缩略图尺寸
 */
export function calculateThumbnailSize(originalWidth, originalHeight, maxSize) {
 ...
 * @returns 预览尺寸
 */
export function getPreviewDimensions(metadata, baseWidth) {
...
export function filterSupportedFiles(files) {
    return files.filter(function (file) { return isSupportedFile(file.name); });
}
