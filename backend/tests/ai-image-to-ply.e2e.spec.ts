...
const TEST_WORKFLOW_ID = "workflow-ai-image-to-ply-e2e";
...
function createSlugFromSourceText(sourceText: string): string {
  return sourceText.replace(/[^a-z0-9]+/gi, "-").replace(/^-+|-+$/g, "").toLowerCase();
}

async function run(): Promise<void> {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "backend-ai-image-to-ply-e2e-"));
  ...
  try {
    ...
    const executor = new AIImageToPlyTaskExecutor(
      ...
      filesRepository,
      {
        async uploadFile(input) {
          ...
          const slug = createSlugFromSourceText(sourceText);

          return withTrackedProviderOperation("upload", slug, async () => ({
            fileName: `uploaded-${slug}.png`,
          }));
        },
        async createWorkflowTask(input) {
          const uploadedFileName = input.nodeInfoList[0]?.fieldValue ?? "unknown";
          const slug = uploadedFileName
            .replace(/^uploaded-/, "")
            .replace(/\.[^.]+$/, "");
          ...
          return withTrackedProviderOperation("query", `${slug}-${nextCount}`, async () => {
            if (slug === "source-ply-failed") {
              return {
                ...
                results: [
                  {
                    fileUrl: `https://example.test/${slug}/output.obj`,
                    fileType: "obj",
                    nodeId: "5",
                    taskCostTime: 11,
                  },
            ...
            return {
              ...
              results: [
                {
                  fileUrl: `https://example.test/${slug}/output.ply`,
                  fileType: "ply",
                  nodeId: "5",
                  taskCostTime: 18,
                },
        ...
        async buildNodeInfoList(input) {
          return {
            ...
            snapshotPath: path.join(
              rootDir,
              `snapshot-${input.uploadedFileName.replace(/[^a-z0-9.-]+/gi, "-")}.json`,
            ),
          };
        },
      },
      ...
      async (input) =>
        withTrackedProviderOperation("download", String(input), async () => {
          ...
          if (url.includes("source-ply-success-1")) {
            return new Response(Buffer.from("ply-success-1-content"), {
              status: 200,
              headers: {
          ...
          if (url.includes("source-ply-success-2")) {
            return new Response(Buffer.from("ply-success-2-content"), {
              status: 200,
              headers: {
                "Content-Type": "application/octet-stream",
              },
    ...
    const sourceFileId1 = await registerReadyFile(
      filesRepository,
      "user-a",
      "source-ply-success-1.png",
      "source-ply-success-1",
    );
    const sourceFileId2 = await registerReadyFile(
      filesRepository,
      "user-a",
      "source-ply-success-2.png",
      "source-ply-success-2",
    );
    const sourceFileIdFailed = await registerReadyFile(
      filesRepository,
      "user-a",
      "source-ply-failed.png",
      "source-ply-failed",
    );
    ...
    const createResult = await executionsService.createExecution(createAIImageToPlyRequest({
      ...
      nodeId: "node-ai-image-to-ply-e2e",
      nodeTitle: "图片转模型端到端",
      groups: [
        {
        ...
        },
        {
          ...
          sourceFileId: sourceFileIdFailed,
        },
      ],
    }));
    ...
    assert.equal(taskSuccess1?.sourceFileId, sourceFileId1);
    assert.equal(taskSuccess1?.provider, "runninghub");
    assert.equal(taskSuccess1?.workflowId, TEST_WORKFLOW_ID);
    assert.equal(taskSuccess1?.workflowTemplateKey, AI_IMAGE_TO_PLY_WORKFLOW_TEMPLATE_KEY);
    assert.equal(taskSuccess1?.providerTaskId, "rh-task-source-ply-success-1");
    assert.equal(taskSuccess1?.providerClientId, "rh-client-source-ply-success-1");
    assert.equal(taskSuccess1?.resultFile?.fileType, "ply");
    ...
    assert.equal(taskSuccess2?.sourceFileId, sourceFileId2);
    assert.equal(taskSuccess2?.providerTaskId, "rh-task-source-ply-success-2");
    assert.equal(taskSuccess2?.providerClientId, "rh-client-source-ply-success-2");
    assert.equal(taskSuccess2?.resultFile?.fileType, "ply");
    assert.ok(taskSuccess2?.resultFileId);

    assert.equal(taskFailed?.status, "failed");
    assert.equal(taskFailed?.groupId, "group-failed");
    assert.equal(taskFailed?.sourceFileId, sourceFileIdFailed);
    assert.equal(taskFailed?.resultFileId, null);
    assert.equal(taskFailed?.lastErrorCode, "VALIDATION_ERROR");
    assert.equal(taskFailed?.providerTaskId, "rh-task-source-ply-failed");
    assert.equal(taskFailed?.providerClientId, "rh-client-source-ply-failed");
    assert.ok(taskFailed?.lastErrorMessage?.includes("ply"));
    ...
    assert.equal(success1Result?.buffer.toString("utf8"), "ply-success-1-content");
    assert.equal(success2Result?.buffer.toString("utf8"), "ply-success-2-content");
    ...
    assert.equal(uploadRecords.length, 3);
    assert.equal(createRecords.length, 3);
    assert.ok(queryRecords.length >= 5);
