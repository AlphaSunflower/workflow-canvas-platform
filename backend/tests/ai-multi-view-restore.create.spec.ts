import assert from "node:assert/strict";
import { createHash } from "node:crypto";
import fs from "node:fs/promises";
import os from "node:os";
import path from "node:path";

import {
  AI_MULTI_VIEW_RESTORE_OUTPUT_FILE_TYPE,
  AI_MULTI_VIEW_RESTORE_OUTPUT_NODE_ID,
  AI_MULTI_VIEW_RESTORE_REFERENCE_INPUT_FIELD_NAME,
  AI_MULTI_VIEW_RESTORE_REFERENCE_INPUT_NODE_ID,
  AI_MULTI_VIEW_RESTORE_RENDER_INPUT_FIELD_NAME,
  AI_MULTI_VIEW_RESTORE_RENDER_INPUT_NODE_ID,
  AI_MULTI_VIEW_RESTORE_WORKFLOW_ID,
  AI_MULTI_VIEW_RESTORE_WORKFLOW_TEMPLATE_KEY,
} from "../shared/src/index.ts";
import { FilesRepository } from "../api/src/modules/files/files.repository.ts";
import {
  buildCreateExecutionStoreInput,
  validateExecutionCreateRequest,
} from "../api/src/modules/executions/execution-node.registry.ts";
import { ExecutionsRepository } from "../api/src/modules/executions/executions.repository.ts";
import { ExecutionsService } from "../api/src/modules/executions/executions.service.ts";
import { createAIMultiViewRestoreRequest } from "./helpers/execution-request.fixture.ts";

async function registerReadyFile(
  filesRepository: FilesRepository,
  userId: string,
  originalName: string,
  content: string,
): Promise<string> {
  const buffer = Buffer.from(content);
  const sha256 = createHash("sha256").update(buffer).digest("hex");

  const registerResult = await filesRepository.registerFile({
    userId,
    sha256,
    size: buffer.length,
    mimeType: "image/png",
    originalName,
    fileType: "image",
    sourceType: "input",
  });

  if (registerResult.uploadRequired && registerResult.uploadId) {
    await filesRepository.uploadFile(
      registerResult.uploadId,
      buffer.toString("base64"),
    );
  }

  return registerResult.file.fileId;
}

function runValidationScenarios(): void {
  const validated = validateExecutionCreateRequest({
    userId: " user-a ",
    workflowId: AI_MULTI_VIEW_RESTORE_WORKFLOW_ID,
    nodeType: "aiMultiViewRestore",
    taskType: "multi-view-restore",
    executionMode: "legacy-grouped-task",
    nodeId: " node-multi-view-restore ",
    nodeTitle: " Multi View Restore ",
    groups: [
      {
        groupId: " group-1 ",
        renderFileId: " render-file-1 ",
        referenceFileId: " reference-file-1 ",
      },
    ],
  });

  assert.equal(validated.nodeType, "aiMultiViewRestore");
  assert.equal(validated.taskType, "multi-view-restore");
  assert.equal(validated.executionMode, "legacy-grouped-task");
  assert.equal(validated.nodeId, "node-multi-view-restore");
  assert.equal(validated.nodeTitle, "Multi View Restore");
  assert.equal(validated.groups[0]?.groupId, "group-1");
  assert.equal(validated.groups[0]?.renderFileId, "render-file-1");
  assert.equal(validated.groups[0]?.referenceFileId, "reference-file-1");

  const storeInput = buildCreateExecutionStoreInput(validated);

  assert.equal(storeInput.run.nodeType, "aiMultiViewRestore");
  assert.equal(storeInput.run.taskType, "multi-view-restore");
  assert.equal(storeInput.run.provider, "runninghub");
  assert.equal(storeInput.run.userId, null);
  assert.equal(storeInput.run.workflowId, AI_MULTI_VIEW_RESTORE_WORKFLOW_ID);
  assert.equal(storeInput.tasks.length, 1);
  assert.equal(storeInput.tasks[0]?.provider, "runninghub");
  assert.equal(storeInput.tasks[0]?.model, null);
  assert.equal(storeInput.tasks[0]?.workflowId, AI_MULTI_VIEW_RESTORE_WORKFLOW_ID);
  assert.deepEqual(storeInput.tasks[0]?.input, {
    renderFileId: "render-file-1",
    referenceFileId: "reference-file-1",
    workflowId: AI_MULTI_VIEW_RESTORE_WORKFLOW_ID,
    workflowTemplateKey: AI_MULTI_VIEW_RESTORE_WORKFLOW_TEMPLATE_KEY,
    renderInputNodeId: AI_MULTI_VIEW_RESTORE_RENDER_INPUT_NODE_ID,
    renderInputFieldName: AI_MULTI_VIEW_RESTORE_RENDER_INPUT_FIELD_NAME,
    referenceInputNodeId: AI_MULTI_VIEW_RESTORE_REFERENCE_INPUT_NODE_ID,
    referenceInputFieldName: AI_MULTI_VIEW_RESTORE_REFERENCE_INPUT_FIELD_NAME,
    outputNodeId: AI_MULTI_VIEW_RESTORE_OUTPUT_NODE_ID,
    outputFileType: AI_MULTI_VIEW_RESTORE_OUTPUT_FILE_TYPE,
  });

  assert.throws(() => {
    validateExecutionCreateRequest({
      workflowId: AI_MULTI_VIEW_RESTORE_WORKFLOW_ID,
      nodeType: "aiMultiViewRestore",
      taskType: "image-to-ply",
      executionMode: "legacy-grouped-task",
      groups: [
        {
          groupId: "group-1",
          renderFileId: "render-file-1",
          referenceFileId: "reference-file-1",
        },
      ],
    } as never);
  }, /INVALID_TASK_TYPE/);

  assert.throws(() => {
    validateExecutionCreateRequest({
      workflowId: AI_MULTI_VIEW_RESTORE_WORKFLOW_ID,
      nodeType: "aiMultiViewRestore",
      taskType: "multi-view-restore",
      executionMode: "mock",
      groups: [
        {
          groupId: "group-1",
          renderFileId: "render-file-1",
          referenceFileId: "reference-file-1",
        },
      ],
    } as never);
  }, /INVALID_EXECUTION_MODE/);

  assert.throws(() => {
    validateExecutionCreateRequest({
      workflowId: AI_MULTI_VIEW_RESTORE_WORKFLOW_ID,
      nodeType: "aiMultiViewRestore",
      taskType: "multi-view-restore",
      executionMode: "legacy-grouped-task",
      groups: [],
    } as never);
  }, /INVALID_GROUPS/);

  assert.throws(() => {
    validateExecutionCreateRequest({
      workflowId: AI_MULTI_VIEW_RESTORE_WORKFLOW_ID,
      nodeType: "aiMultiViewRestore",
      taskType: "multi-view-restore",
      executionMode: "legacy-grouped-task",
      groups: [
        {
          groupId: "group-1",
          renderFileId: "   ",
          referenceFileId: "reference-file-1",
        },
      ],
    } as never);
  }, /INVALID_RENDER_FILE_ID:0/);

  assert.throws(() => {
    validateExecutionCreateRequest({
      workflowId: AI_MULTI_VIEW_RESTORE_WORKFLOW_ID,
      nodeType: "aiMultiViewRestore",
      taskType: "multi-view-restore",
      executionMode: "legacy-grouped-task",
      groups: [
        {
          groupId: "group-1",
          renderFileId: "render-file-1",
          referenceFileId: "   ",
        },
      ],
    } as never);
  }, /INVALID_REFERENCE_FILE_ID:0/);

  assert.throws(() => {
    validateExecutionCreateRequest({
      workflowId: AI_MULTI_VIEW_RESTORE_WORKFLOW_ID,
      nodeType: "aiMultiViewRestore",
      taskType: "multi-view-restore",
      executionMode: "legacy-grouped-task",
      groups: [
        {
          groupId: "group-1",
          renderFileId: "same-file",
          referenceFileId: "same-file",
        },
      ],
    } as never);
  }, /DUPLICATE_GROUP_INPUT_FILE:0/);
}

async function runCreateExecutionScenarios(): Promise<void> {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "backend-ai-multi-view-restore-create-"));

  try {
    const filesRepository = new FilesRepository(rootDir);
    const executionsRepository = new ExecutionsRepository(rootDir);
    const executionsService = ExecutionsService.fromRoot(rootDir);

    const renderFileId1 = await registerReadyFile(
      filesRepository,
      "user-a",
      "render-image-1.png",
      "render-image-content-1",
    );
    const referenceFileId1 = await registerReadyFile(
      filesRepository,
      "user-a",
      "reference-image-1.png",
      "reference-image-content-1",
    );
    const renderFileId2 = await registerReadyFile(
      filesRepository,
      "user-a",
      "render-image-2.png",
      "render-image-content-2",
    );
    const referenceFileId2 = await registerReadyFile(
      filesRepository,
      "user-a",
      "reference-image-2.png",
      "reference-image-content-2",
    );

    const result = await executionsService.createExecution(createAIMultiViewRestoreRequest({
      userId: "user-a",
      workflowId: AI_MULTI_VIEW_RESTORE_WORKFLOW_ID,
      nodeType: "aiMultiViewRestore",
      taskType: "multi-view-restore",
      executionMode: "legacy-grouped-task",
      nodeId: "node-multi-view-restore",
      nodeTitle: "Multi View Restore",
      groups: [
        {
          groupId: "group-1",
          renderFileId: renderFileId1,
          referenceFileId: referenceFileId1,
        },
        {
          groupId: "group-2",
          renderFileId: renderFileId2,
          referenceFileId: referenceFileId2,
        },
      ],
    }));

    assert.ok(result.runId);
    assert.equal(result.status, "queued");
    assert.equal(result.nodeType, "aiMultiViewRestore");
    assert.equal(result.taskType, "multi-view-restore");
    assert.equal(result.tasks.length, 2);
    assert.equal(result.tasks[0]?.groupId, "group-1");
    assert.equal(result.tasks[0]?.groupOrder, 1);
    assert.equal(result.tasks[1]?.groupId, "group-2");
    assert.equal(result.tasks[1]?.groupOrder, 2);

    const task1 = await executionsRepository.getTaskById(result.tasks[0]!.taskId);
    const task2 = await executionsRepository.getTaskById(result.tasks[1]!.taskId);

    assert.ok(task1);
    assert.ok(task2);
    assert.equal(task1?.nodeType, "aiMultiViewRestore");
    assert.equal(task1?.taskType, "multi-view-restore");
    assert.equal(task1?.provider, "runninghub");
    assert.equal(task1?.model, null);
    assert.deepEqual(task1?.input, {
      renderFileId: renderFileId1,
      referenceFileId: referenceFileId1,
      workflowId: AI_MULTI_VIEW_RESTORE_WORKFLOW_ID,
      workflowTemplateKey: AI_MULTI_VIEW_RESTORE_WORKFLOW_TEMPLATE_KEY,
      renderInputNodeId: AI_MULTI_VIEW_RESTORE_RENDER_INPUT_NODE_ID,
      renderInputFieldName: AI_MULTI_VIEW_RESTORE_RENDER_INPUT_FIELD_NAME,
      referenceInputNodeId: AI_MULTI_VIEW_RESTORE_REFERENCE_INPUT_NODE_ID,
      referenceInputFieldName: AI_MULTI_VIEW_RESTORE_REFERENCE_INPUT_FIELD_NAME,
      outputNodeId: AI_MULTI_VIEW_RESTORE_OUTPUT_NODE_ID,
      outputFileType: AI_MULTI_VIEW_RESTORE_OUTPUT_FILE_TYPE,
    });
    assert.deepEqual(task2?.input, {
      renderFileId: renderFileId2,
      referenceFileId: referenceFileId2,
      workflowId: AI_MULTI_VIEW_RESTORE_WORKFLOW_ID,
      workflowTemplateKey: AI_MULTI_VIEW_RESTORE_WORKFLOW_TEMPLATE_KEY,
      renderInputNodeId: AI_MULTI_VIEW_RESTORE_RENDER_INPUT_NODE_ID,
      renderInputFieldName: AI_MULTI_VIEW_RESTORE_RENDER_INPUT_FIELD_NAME,
      referenceInputNodeId: AI_MULTI_VIEW_RESTORE_REFERENCE_INPUT_NODE_ID,
      referenceInputFieldName: AI_MULTI_VIEW_RESTORE_REFERENCE_INPUT_FIELD_NAME,
      outputNodeId: AI_MULTI_VIEW_RESTORE_OUTPUT_NODE_ID,
      outputFileType: AI_MULTI_VIEW_RESTORE_OUTPUT_FILE_TYPE,
    });

    await assert.rejects(() => {
      return executionsService.createExecution(createAIMultiViewRestoreRequest({
        userId: "user-a",
        workflowId: AI_MULTI_VIEW_RESTORE_WORKFLOW_ID,
        nodeType: "aiMultiViewRestore",
        taskType: "multi-view-restore",
        executionMode: "legacy-grouped-task",
        nodeId: "node-multi-view-restore",
        nodeTitle: "Multi View Restore",
        groups: [
          {
            groupId: "group-3",
            renderFileId: "file-missing-render",
            referenceFileId: referenceFileId1,
          },
        ],
      }));
    }, /FILE_NOT_FOUND:file-missing-render/);

    await assert.rejects(() => {
      return executionsService.createExecution(createAIMultiViewRestoreRequest({
        userId: "user-a",
        workflowId: AI_MULTI_VIEW_RESTORE_WORKFLOW_ID,
        nodeType: "aiMultiViewRestore",
        taskType: "multi-view-restore",
        executionMode: "legacy-grouped-task",
        nodeId: "node-multi-view-restore",
        nodeTitle: "Multi View Restore",
        groups: [
          {
            groupId: "group-4",
            renderFileId: renderFileId1,
            referenceFileId: "file-missing-reference",
          },
        ],
      }));
    }, /FILE_NOT_FOUND:file-missing-reference/);
  } finally {
    await fs.rm(rootDir, { recursive: true, force: true });
  }
}

async function run(): Promise<void> {
  runValidationScenarios();
  await runCreateExecutionScenarios();
}

void run();
